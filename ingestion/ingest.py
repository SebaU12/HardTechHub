...
EVENT_TYPES = ["PRODUCT_VIEW", "PRODUCT_SEARCH", "ADD_TO_CART", "REMOVE_FROM_CART", "ORDER_CREATED"]
...
def s3_key(now: datetime) -> str:
    return (
        f"raw/events/year={now.year}/month={now.month:02d}"
...
def upload_processed(s3, events: list[dict], now: datetime) -> None:
    ...
    key = (
        f"processed/events/year={now.year}/month={now.month:02d}"
